...
// 是否显示表单弹窗
const formIsOpen = ref(appStore.getFormIsOpen)
const formIsOpenChange = (show: boolean) => {
  appStore.setFormIsOpen(show)
}
...
<template>
  <div :class="prefixCls">
    ...
    <!-- <div class="flex justify-between items-center">
      <span class="text-14px">表单交互方式为弹窗</span>
      <ElSwitch v-model="formIsOpen" @change="formIsOpenChange" />
    </div> -->
  </div>
</template>
