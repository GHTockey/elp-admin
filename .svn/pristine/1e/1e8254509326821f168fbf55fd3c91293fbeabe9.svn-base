...
import { fileToBinary, getFullImageUrl, objToFormData, uploadFile } from '@/utils';
...
const handleCreated = (editor: IDomEditor) => {
  ...
  if (!editor.getAllMenuKeys()?.includes('viewHTML')) {
    Boot.registerMenu({
      key: 'viewHTML',
      factory() {
        return {
...
editorConfig.value.MENU_CONF['uploadImage'] = {
  ...
  async customUpload(file: File, insertFn: InsertFnType) {  // TS 语法
    ...
    if (response.code === SUCCESS_CODE) {
      ...
      let url = getFullImageUrl(response.data.file);
      let alt = file.name;
      let href = url;
      // 最后插入图片
