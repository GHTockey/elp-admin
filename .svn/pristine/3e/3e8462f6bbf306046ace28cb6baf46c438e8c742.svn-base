...
export const pathResolve = (parentPath: string, path: string) => {
  ...
  // 如果路径以'/'开头或路径为空，则子路径为路径本身，否则在路径前加上'/'
  const childPath = path.startsWith('/') || !path ? path : `/${path}`
  // console.log('childPath', childPath);

  // 合并父路径和子路径，并替换掉多余的斜杠，最后去掉首尾空格(只去除路径中的双斜杠，不影响查询参数)
  // console.log(`${parentPath}${childPath}`.replace(/\/{2,}(?=[^?]*\?)/g, '/').trim());
  // return `${parentPath}${childPath}`.replace(/\/{2,}(?=[^?]*\?)/g, '/').trim()

  // console.log( `${parentPath}${childPath}`.replace(/(?<!:)\/\/+/g, '/').trim());
