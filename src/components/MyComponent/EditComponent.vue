<template>
    ...
    <el-tabs v-else v-model="activeTab" type="border-card">
        <el-tab-pane v-for="(fields, tag) in groupedFields" :key="tag" :label="tag">
            <el-form :model="tableDataForm" v-loading="loading" :rules="rules" ref="tableFormEl" label-width="180px">
                <template v-for="(item, key) in fields" :key="key">
                    ...
                    <el-form-item :label="item.vi_name" :prop="key" class="w-[90%]" v-if="key != 'unknown_file_accept'">
                        ...
                        <template v-else-if="FieldTypeChecker.isPopupField(item)">
                            <div class="flex">
                                <el-input v-model="tableDataForm[key]" :placeholder="item.placeholder_search || '请选择'"
                                    :disabled="!SacManage.has_auth_edit_field(item)">
                                    <!-- 数据回显 -->
...
function initRules() {
    for (const key in tableData.value?.form_fields) {
        if (tableData.value?.form_fields.hasOwnProperty(key)) {
            ...
            // console.log('field', field);
            // if (field.validate_js?.includes('required') || isRequired(field)) { // 必填项
            if (field.validate_js?.includes('required')) { // 必填项
